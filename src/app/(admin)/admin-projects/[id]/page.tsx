...
import { ChevronRight, Building2, Trash2, Pencil, X } from 'lucide-react'
...
export default function ProjectDetailPage() {
  ...
  useEffect(() => {
    fetch(`/api/admin/projects/${id}`, { cache: 'no-store' })
      ...
      .then(d => {
        ...
        setMemberUserIds(d.members?.map((m: any) => m.user.id) || [])
        setLoading(false)
      })
    fetch('/api/admin/components', { cache: 'no-store' }).then(r => r.json()).then(d => setComponents(d))
    fetch('/api/admin/users', { cache: 'no-store' }).then(r => r.json()).then(d => setUsers(d))
  }, [id])
  ...
  function toggleMember(userId: string) {
    setMemberUserIds(ids => ids.includes(userId) ? ids.filter(i => i !== userId) : [...ids, userId])
  }
    ...
    <div className="animate-fade-in space-y-6">
      ...
      <div className="rounded-2xl border border-border bg-card p-8 space-y-8">
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            className="h-10 w-10 rounded-full border border-gray-200 flex items-center justify-center shrink-0 hover:bg-gray-50 transition-colors"
          >
      ...
      {showMemberModal && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-[70]">
          <div className="bg-white rounded-2xl w-full max-w-md mx-4 shadow-2xl overflow-hidden flex flex-col max-h-[80vh]">
            ...
            <div className="flex-1 overflow-y-auto px-6 py-4 space-y-2">
              ...
              {activeUsers.map(u => (
                <label key={u.id} className="flex items-center gap-3 px-3 py-2.5 rounded-lg border border-gray-100 hover:bg-gray-50 cursor-pointer transition-colors">
                  <input
                    type="checkbox"
            ...
            <div className="flex justify-end gap-3 px-6 py-4 border-t border-gray-100">
              <button onClick={() => setShowMemberModal(false)}
                className="border border-gray-200 text-gray-700 text-sm px-5 py-2 rounded-lg font-medium hover:bg-gray-50 transition-colors">
                Cancel
              </button>
              <button onClick={saveMembers}
                className="bg-primary text-white text-sm px-5 py-2 rounded-lg font-medium hover:bg-primary/90 transition-colors">
                Save
              </button>
            </div>
          </div>
