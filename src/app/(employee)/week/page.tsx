...
export default function WeekPage() {
  ...
  return (
    <div className="animate-fade-in">
      <div className="mb-6"><h1 className="text-2xl font-bold">This Week</h1><p className="text-muted-foreground text-sm mt-0.5">{formatHours(totalWeekHours)} total</p></div>
      <div className="flex items-center justify-between mb-4">
        ...
        <span className="text-sm font-medium">{format(weekStart,'d MMM')} - {format(weekEnd,'d MMM yyyy')}</span>
      ...
      <div className="flex items-center justify-between mb-3">
        <h2 className="font-semibold text-sm">{format(new Date(selectedDate+'T12:00:00'),'EEEE, d MMMM')}</h2>
        <Link href={`/log?date=${selectedDate}`}><Button size="sm" variant="outline"><Plus className="h-3.5 w-3.5 mr-1" /> Add</Button></Link>
      </div>
      {loading ? <div className="space-y-3">{[1,2].map(i=><div key={i} className="h-16 rounded-2xl bg-muted animate-pulse" />)}</div>
      : selectedEntries.length === 0 ? <Card><CardContent className="py-10 text-center"><p className="text-sm text-muted-foreground mb-3">No entries for this day</p><Link href={`/log?date=${selectedDate}`}><Button size="sm">Log Time</Button></Link></CardContent></Card>
