  ...
  return (
    <div className="animate-fade-in">
      <div className="mb-6">
        ...
        <p className="text-muted-foreground text-sm mt-0.5">{t.projectsSubtitle}</p>
      </div>

      {loading ? <div className="text-center text-muted-foreground text-sm py-12">{t.loadingText}</div>
      : projects.length === 0 ? <div className="text-center text-muted-foreground text-sm py-12">{t.noProjects}</div>
      : (
        <div className="space-y-3">
          {projects.map(p => (
            <Card key={p.id}>
