...
function ResetPasswordForm() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (res.ok) { setState('success'); setTimeout(() => router.push('/login'), 1500) } else { setErrors({ form: 'Link expired. Request a new one.' }); setLoading(false) }
  }
  if (state === 'loading') return <Card><CardContent className="pt-6 py-12 text-center"><p className="text-sm text-muted-foreground">Validating...</p></CardContent></Card>
  if (state === 'invalid') return <Card><CardContent className="pt-6 py-12 text-center"><AlertCircle className="h-10 w-10 text-destructive mx-auto mb-4" /><h2 className="font-semibold mb-2">Link expired</h2><Button variant="outline" onClick={() => router.push('/forgot-password')}>Request new link</Button></CardContent></Card>
  if (state === 'success') return <Card><CardContent className="pt-6 py-12 text-center"><h2 className="font-semibold mb-2">Password updated</h2><p className="text-sm text-muted-foreground">Redirecting...</p></CardContent></Card>
  return (
